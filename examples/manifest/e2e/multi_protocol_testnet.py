#!/usr/bin/env python3
"""
Test multiple protocol endpoints deployment on Akash Network testnet.

This test demonstrates:
- HTTP endpoint (TCP port 80) - SHARED_HTTP (kind 0)
- UDP endpoint (UDP port 12345) - RANDOM_PORT (kind 1)
- Correct endpoint kind determination
- Multiple expose configurations with different protocols
"""
...
def run_deployment():
    ...
    print("\n2. Create multi-protocol endpoint deployment")
    sdl_content = '''
version: "2.0"
services:
  game-server:
...
'''
    ...
    deployment = client.deployment.create_deployment(
        ...
        deposit="5000000",
        fee_amount="5000",
        memo="Multi-protocol endpoint test"
    )
    ...
    print("\n" + "=" * 60)
    print("Multi-protocol endpoint deployment complete")
    print(f"Dseq: {dseq}")
    print(f"Provider: {lease['provider_endpoint']}")
    print("\nEndpoint configuration:")
    print("  HTTP (TCP):  Port 80  → SHARED_HTTP (kind 0)")
    print("  UDP:         Port 12345 → RANDOM_PORT (kind 1)")
